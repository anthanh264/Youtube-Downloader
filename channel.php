...
<div class="bg1" style="background: url(./thumbnail.php?type=banner&vid=<?php echo $channel['items'][0]['id'] ?>);background-repeat: no-repeat;background-position: center center;background-size: cover;" >
         ...
         <div class="container pt-2">
           ...
           <p class="kkkkkk pb-1 text-center" style="color: white;">
               <?php
               if(!empty($channel['items'][0]['snippet']['description'])){echo $channel['items'][0]['snippet']['description'];}else{echo 'Không có nội dung! Người dùng kênh này không tải lên bất kỳ nội dung hoặc nội dung kênh có bản quyền, tạm thời không thể xem!...';}?>
               </p>
          <script>
$(document).ready(function(){
 ...
 <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 equipodiv order-md-1 order-lg-1 order-xs-2 order-sm-2">
  <div class="related">
      <p class="font-weight-bold h6 pb-1">Info</p>
       <span class="d-block py-3 fa fa-calendar"> Đăng Ký <?php echo date("Y-m-d", strtotime($channel['items'][0]['snippet']['publishedAt']));?></span>  
       <span class="d-block py-3 fa fa-play"> <?php echo $channel['items'][0]['statistics']['viewCount']?> View</span> 
       <span class="d-block py-3 fa fa-user-circle-o"> <?php echo $channel['items'][0]['statistics']['subscriberCount']?> Subscribe</span>
       <span class="d-block py-3 fa fa-globe"> Đến từ <?php if (array_key_exists('country',$channel['items'][0]['snippet'])) {echo get_country($channel['items'][0]['snippet']['country']);} else {echo 'Sao Hỏa';}?></span> 

  </div>
  <div class="related text-nowrap">
      <p class="font-weight-bold h6 pb-1">Các Kênh Liên Quan</p>
       <?php
 ...
 <div class="col-md-8 col-sm-12 col-xs-12 col-lg-8 related order-md-2 order-lg-2 order-xs-1 order-sm-1">
    <div class="font-weight-bold h6 pb-1">Tất cả video（<?php echo $channel['items'][0]['statistics']['videoCount'] ?> Video）</div> 
